...
<script>
    ...
    $(document).ready(function() {
        $('#myTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('http://kaspdrj.test/kasagustusAjax') }}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false,
        ...
        $('body').on('click', '.btnEdit', function(event) {
            ...
            var id = $(this).data('id');
            $.ajax({
                url: 'http://kaspdrj.test/kasagustusAjax/' + id + '/edit',
                type: 'GET',
    ...
    function simpan(id) {
        ...
        if (id === '') {
            var_url = 'http://kaspdrj.test/kasagustusAjax';
            var_type = 'POST';
        } else {
            // If id is provided, we're editing existing data
            var_url = 'http://kaspdrj.test/kasagustusAjax/' + id;
            var_type = 'PUT';
        }
...
<script>
    ...
    $('body').on('click', '.btnDel', function(e) {
        if (confirm('Hapus data ini?') == true) {
            ...
            $.ajax({
                url: 'http://kaspdrj.test/kasagustusAjax/' + id,
                type: 'DELETE',
                headers: {
